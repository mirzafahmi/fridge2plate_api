...
def get_instructions(db: Session, skip: int=0, limit: int = 100):
    return db.query(Instruction).offset(skip).limit(limit).all()

def get_instruction_by_id(db: Session, instruction_id: UUID):
    return db.query(Instruction).filter(Instruction.id == instruction_id).first()

def get_instructions_by_recipe_id(db: Session, recipe_id: UUID):
    return db.query(Instruction).filter(Instruction.recipe_id == recipe_id).order_by(asc(Instruction.step_number)).all()

def check_instruction_by_step_number_duplication(db: Session, instruction: InstructionCreate):
    return db.query(Instruction).filter_by(recipe_id=instruction.recipe_id, step_number=instruction.step_number).first()

def validate_step_number(db: Session, recipe_id: UUID, new_step_number: int):
    # Get the current highest step_number for the recipe
    max_step_number = (
        db.query(func.max(Instruction.step_number))
...
def post_instruction(db: Session, instruction: InstructionCreate):
    ...
    db_instruction = Instruction(**instruction_data)
