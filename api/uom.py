...
async def read_uom_list(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user), skip: int=0, limit: int = 100):
...
async def read_uom_by_id(*, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user), uom_id: UUID):
...
async def add_uom(*, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user), uom: UOMCreate):
    ...
    result_message = f"{uom.name} as UOM is created successfully"
...
async def change_uom(*,db: Session = Depends(get_db), current_user: dict = Depends(get_current_user), uom_id: UUID, uom: UOMUpdate):
    uom_by_id = get_uom_by_id(db, uom_id)
    ...
    result_message = f"Id {uom_id} as UOM is updated successfully"

    return {"detail": result_message, "uom": uom_update}

@router.delete("/{uom_id}", status_code=status.HTTP_200_OK)
async def remove_uom(*, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user), uom_id: UUID):
    uom_by_id = get_uom_by_id(db, uom_id)
    ...
    result_message = f"Id {uom_id} as UOM is deleted successfully"
